...
<Tabs>
  ...
  <Tab title="Bad">
    ...
    // No analytics integration - can't measure experiment impact

    // ❌ Using events instead of user properties
    analyticsHandler = object : StringbootAnalyticsHandler {
        override fun onExperimentsAssigned(experiments: Map<String, ExperimentAssignment>) {
            // Wrong: Events don't persist across sessions
            firebaseAnalytics.logEvent("experiment_assigned", bundle)
        }
    }
    ```
  </Tab>
