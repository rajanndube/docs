...
For server components, fetch strings server-side:
...
<Note>
The SDK works offline-first, so strings will load progressively even if the initial fetch is slow.
</Note>
...
<Warning>
Use `NEXT_PUBLIC_` prefix for client-side usage. Use regular env vars for server-side only to keep tokens secure.
</Warning>
