...
<Tabs>
  <Tab title="Google Analytics 4">
    ...
    2. Create custom event parameters:
       ...
       - `variant_name`
       - `experiment_name`
    3. View events in **Reports** → **Events** → `stringboot_experiment_assigned`
  </Tab>
  ...
  <Tab title="Mixpanel">
    ...
    1. Go to **Insights** → **Filter by Event** → "Stringboot Experiment Assigned"
    2. View user properties under **Users** → select user → **Properties**
  ...
  <Tab title="Amplitude">
    ...
    1. Go to **Events** → Search for "Stringboot Experiment Assigned"
    2. View user properties under **User Look-Up** → select user
...
2. **Application** → **Local Storage**: `stringboot_device_id` exists
...
<Tabs>
  ...
  <Tab title="Mixpanel">
    ...
    4. Check **Users** → select user → **Properties** for `exp_{key}`
  </Tab>
