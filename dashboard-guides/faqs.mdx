...
<Check>CSV or JSON file prepared with FAQ data</Check>
<Check>File follows the correct format</Check>
...
Here's how your FAQs might be consumed via API:

```javascript
// Fetch FAQs by tag
const faqs = await fetch(
  'https://api.stringboot.com/api/faqs?appId=123&tag=getting-started'
).then(r => r.json());
